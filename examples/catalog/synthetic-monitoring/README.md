...
This inert example installs Synthetic Monitoring for an existing stack and declares one HTTP check plus one browser check. Targets and probe locations are written by the consuming team because the vending platform cannot infer them from a stack request.
...
- Replace both example targets and probe names with reviewed service endpoints and approved execution locations.
