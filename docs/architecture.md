...
| cloud | vended | `GrafanaCloudStackRequest`, `GrafanaPDC`, `GrafanaProjectContent`, `GrafanaStackConsumer`, `GrafanaStackLadder` | Core owns Stack, StackServiceAccount, StackServiceAccountRotatingToken, AccessPolicy, AccessPolicyRotatingToken, optional PluginInstallation, and the three product global singletons selected by `spec.products`. Product configuration is not part of the stack request. `GrafanaStackConsumer` observes an existing Stack without lifecycle ownership and owns only its distinct platform-authorized AccessPolicy and rotating token. |
...
| enterprise | vended | `GrafanaCloudStackRequest`, `GrafanaCustomRoleBinding`, `GrafanaDatasourceAccess`, `GrafanaProjectContent`, `GrafanaProvisioningConnection`, `GrafanaStackLadder`, `GrafanaTeamAccess` | Core optionally owns Report; access APIs own Role, RoleAssignment, and RoleAssignmentItem. `GrafanaDatasourceAccess` owns one DataSource, its whole permission set, and aggregated LBAC tree. `GrafanaProvisioningConnection` owns the Securevalue side of its secret-store-backed credential bridge. Team Sync external-group mapping is supported. SCIM is explicitly rejected because its Team ownership conflicts with that model; Keeper remains separate. |
...
| oss | vended | `GrafanaCloudStackRequest`, `GrafanaContentAccessPolicy`, `GrafanaCustomRoleBinding`, `GrafanaDatasourceAccess`, `GrafanaPDC`, `GrafanaProjectContent`, `GrafanaProvisioningConnection`, `GrafanaProvisioningRepository`, `GrafanaServiceAccounts`, `GrafanaStackLadder`, `GrafanaTeamAccess` | Core owns Folder, Dashboard, OrganizationPreferences, and SsoSettings; access APIs own Team, FolderPermission, and DashboardPermission. `GrafanaProvisioningConnection` and `GrafanaProvisioningRepository` together form an opt-in preview Git subtree route with a secret-store-backed connection. Inventory observes folders, dashboards, teams, users, and library panels; playlists and annotations remain separate; `GrafanaServiceAccounts` vends additional service accounts. |
