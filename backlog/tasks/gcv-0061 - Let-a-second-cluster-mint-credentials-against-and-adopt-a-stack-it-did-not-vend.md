...
- [ ] #2 A cluster holding no local claim for a stack admits an AccessPolicy naming that stack and renders a rotating-token child bound to it, proven against the real API server; live minting is outside this repository's no-live-contact constraint and is recorded as downstream proof
